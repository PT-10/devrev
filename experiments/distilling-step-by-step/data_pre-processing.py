...
# with open('datasets/svamp/SVAMP2.json', 'w') as file:
...
#     d = {"Reasoning": data[i]["Reasoning"]}
#     data[i]['Output'].append(d)
#     data[i].pop("Reasoning")
    
# with open('datasets/svamp/SVAMP.json', 'w') as file:
#     json.dump(data, file, indent=2)

# with open('svamp_train_temp.jsonl', 'w') as file:
#     for i in range(n):
#         dict = {}
#         dict["input"] = data[i]["Query"]
#         dict["label"] = data[i]["Output"]
#         json_line = json.dumps(dict)
#         file.write(json_line + '\n')

# with open('svamp_train_temp.json', 'w') as file:
#     for i in range(n):
#         data[i]['label'] = str(data[i]['label'])
#         json_line = json.dumps(data[i])
#         file.write(json_line + '\n')

for i in range(n):
    data[i]['Output'] = data[i]['Output'][:-1]

with open('SVAMP1.json', 'w') as file:
    json.dump(data, file, indent=2)